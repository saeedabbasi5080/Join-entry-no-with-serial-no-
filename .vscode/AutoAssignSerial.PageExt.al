...
//             Error('This function only works for Sales Orders.');
//             exit;
//         end;
...
//             Error('All required quantity has already been assigned. No additional serial numbers needed.');
//             exit;
//         end;
...
//         // Only count records with actual serial numbers assigned
//         ExistingTrackingSpec.SetFilter("Serial No.", '<>%1', '');
...
//                 // Only proceed if serial number is available AND not already in current tracking
//                 if IsSerialNumberAvailableForAssignment(TrackingSpec, ItemLedgEntry, ItemNo) then begin
//                     // Double check: make sure serial doesn't exist in current tracking
//                     if not DoesSerialNumberExistInCurrentTracking(TrackingSpec, ItemLedgEntry."Serial No.") then begin
...
//     begin
//         // First check if serial already exists in current tracking lines (most important check)
//         if DoesSerialNumberExistInCurrentTracking(TrackingSpec, ItemLedgEntry."Serial No.") then
//             exit(false);

//         // Then check if serial is reserved for another document
//         if IsSerialNumberReservedElsewhere(TrackingSpec, ItemLedgEntry, ItemNo) then
//             exit(false);

//         // If both checks pass, serial is available
//         exit(true);
//     end;
...
//         // Exclude current source document to avoid checking our own reservations
...
//         // Make sure we're checking records that actually have this serial number
//         ExistingTrackingSpec.SetFilter("Serial No.", '<>%1', '');
...
//                 Message('No additional available serials found. %1 serials still needed.', RemainingQty)
//             else
//                 Message('All required serial numbers are already assigned.');
//         end else
//             if FoundCount < RemainingQty then
